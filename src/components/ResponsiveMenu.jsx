...
const ResponsiveMenu = ({ openNav, setOpenNav }) => {
    ...
    return (
        <div className={`${openNav ? "left-0" : "-left-[100%]"} fixed bottom-0 top-0 z-20 flex h-screen w-[75%] flex-col justify-between bg-white px-8 pb-6 pt-16 text-black md:hidden rounded-r-xl shadow-md transition-all`}>
            <div>
                ...
                <nav className='mt-12'>
                    <ul className='flex flex-col gap-7 text-2xl font-semibold'>
                        ...
                        <Link to={'/Cart'} onClick={() => setOpenNav(false)} className="cursor-pointer"><li>Cart</li></Link>
            ...
            <div className="mt-4">
                <SignedOut>
                    <SignInButton className="bg-red-500 text-white px-4 py-2 rounded-md cursor-pointer w-full text-center">
                        Sign in
                    </SignInButton>
                </SignedOut>
            </div>
